...
def _openfood_is_argentina_product(raw):
    tags = raw.get('countries_tags') or []
    if isinstance(tags, str):
        tags = [tags]
    countries = raw.get('countries') or ''
    values = [str(v).lower() for v in tags]
    values.append(str(countries).lower())
    return any('argentina' in value or value in ('ar', 'en:ar', 'es:ar') for value in values)


def _openfood_product_payload(raw, fallback_barcode='', require_argentina=False):
    from services.openfood_importer import _map_category

    if require_argentina and not _openfood_is_argentina_product(raw):
        return None
    ...
    return {
        ...
        'unit': 'Unidad',
        'por_peso': 0,
        'argentina': _openfood_is_argentina_product(raw),
        'exists': bool(db.q(
...
def _openfood_products_from_search(term, limit=12):
    ...
    fields = 'code,product_name,product_name_es,abbreviated_product_name,brands,categories_tags,countries_tags,countries'
    urls = []
    for base_params in (
        {
            'search_terms': term,
    ...
    for url in urls:
        ...
        for raw in data.get('products', []):
            product = _openfood_product_payload(raw, require_argentina=True)
...
def api_openfood_search():
    ...
    try:
        ...
        if term.isdigit() and len(term) >= 7:
            ...
            data = _openfood_fetch_json(url)
            if data.get('status') == 1:
                product = _openfood_product_payload(data.get('product', {}), term, require_argentina=True)
                if product:
                    products.append(product)
        else:
    ...
    try:
        ...
        try:
            ...
            if off_data.get('status') == 1:
                product = _openfood_product_payload(off_data.get('product', {}), barcode, require_argentina=True)
        except Exception:
            product = None

        if not product:
            if not data.get('argentina'):
                return jsonify({'ok': False, 'msg': 'El producto no pertenece a la base argentina de OpenFoodFacts.'})
            product = {
                'barcode': barcode,
