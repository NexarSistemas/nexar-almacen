...
def licencia():
    ...
    return render_template('licencia.html', status=status, machine_id=machine_id)

@app.route('/licencia/activar', methods=['POST'])
@admin_required
def licencia_activar():
    codigo = request.form.get('codigo','').strip()
    if db.activar_licencia(codigo):
        flash('🎉 ¡Licencia activada exitosamente! Sistema desbloqueado al completo.', 'success')
    else:
        flash('❌ Código de activación inválido. Verificá el código e intentá nuevamente.', 'danger')
