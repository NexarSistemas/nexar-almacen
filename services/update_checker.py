...
DEFAULT_REPO = "NexarSistemas/nexar-almacen"
...
def _asset_matches_platform(name: str) -> bool:
    ...
    if _is_windows():
        return normalized.startswith("nexaralmacen_") and normalized.endswith("_setup.exe")
    return normalized.startswith("nexar-almacen_") and normalized.endswith("_amd64.deb")
...
def check_latest_release(current_version: str) -> dict[str, Any]:
    ...
    response = requests.get(
        _release_api_url(),
        headers={"Accept": "application/vnd.github+json", "User-Agent": "NexarAlmacen"},
    ...
    return {
        ...
        "name": release.get("name") or f"Nexar Almacen v{latest}",
